...
int sound_level = 25; //difference between initial sound and final sound in db - to be considered significant sound change
...
void loop()
{
  while(digitalRead(on_switch) == 1) //if device switch is on
  {
    ...
    //measure sound level initially
    float initial_sound = sound(); 
    
    if(speed < 1) //if speed is less than 1km/hr
    {
      digitalWrite(led_on, LOW);
      digitalWrite(led_transition, HIGH); //turn on transition led
      delay(timer); //wait for 20s

      //check speed again after 20s
      Speed();
      speed = (RPM*kkbanspd);

      float final_sound = sound(); //measure sound level after 20s
